...
#         features_multiscale.requires_grad=False
        ...
        if self.eval_only:
            ...
            ret_dic['individual_feat'] = individual_feat.view(B, N*NFB)
            return ret_dic
        else:
...
#         self.nl_emb_1=nn.LayerNorm([NFB])
    ...
    def forward(self,batch_data):
        # images_in, boxes_in, images_person_in, bboxes_num_in = batch_data
        images_in = batch_data['images_in']
        boxes_in = batch_data['boxes_in']
        images_person_in = batch_data['images_person_in']
        bboxes_num_in = batch_data['bboxes_num_in']
        ...
        if self.eval_only:
            individual_feat, _ = torch.max(boxes_features_all.reshape(B,T,MAX_N,-1), dim=1)
            individual_feat = individual_feat.view(B, -1)
            group_feat = boxes_states_pooled
            ret_dic = {}
            ret_dic['actions'] = actions_scores
            ret_dic['group_feat'] = group_feat
            ret_dic['individual_feat'] = individual_feat
            return ret_dic
        else:
            return actions_scores, activities_scores